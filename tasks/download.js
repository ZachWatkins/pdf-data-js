...
const options = {
  ...
  dest:
    process.env.npm_config_dest || process.env.npm_package_config_download_dest,
  force:
    process.env.npm_config_force ||
    process.env.npm_package_config_download_force,
}
const dest = 'data'
...
function download({ url, dest, force }) {
  ...
  if (typeof force === 'undefined' && fs.existsSync(dest)) {
    errors.push(
      new Error(`File exists: ${path.basename(dest)}. Use --force to override.`)
    )
  }

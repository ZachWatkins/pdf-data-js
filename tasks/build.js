...
class WorkBook {
  ...
  build() {
    ...
    this.sheets.forEach((sheet) => {
      ...
      if (!sources[source]) {
        sources[source] = XLSX.readFileSync(
      ...
      fs.writeFile(
        ...
        `const ${this.name.replace(/\s+/, '')}_${sheetName.replace(
          /\s+/,
          ''
        )} = ${jsonData}`,
        { encoding: 'utf8' },
...
function WriteJsonFile({ data, file, pretty, force }) {
  ...
  const json = !pretty ? JSON.stringify(data) : JSON.stringify(data, null, 4)
